...
class TestInstanceConfigDefaults:
    def test_defaults(self):
        ...
        assert c.port == 8000, (
            "instance-config-port: default port MUST be 8000; "
            f"got {c.port}"
        )
        assert c.reload is False, (
            "instance-config-reload: default reload MUST be False; "
            f"got {c.reload!r}"
        )


class TestInstanceConfigPrecedence:
    def test_cli_beats_env_beats_yaml_beats_default(self, tmp_path, monkeypatch):
    ...
    def test_env_beats_yaml(self, tmp_path, monkeypatch):
        ...
        assert c.port == 8888, (
            "instance-config-resolve: env MUST win over YAML; "
            f"got port={c.port}"
        )
    ...
    def test_yaml_beats_default(self, tmp_path):
        ...
        c = resolve(sidestage_dir=sd)
        assert c.port == 7777, (
            "instance-config-resolve: YAML MUST win over defaults; "
            f"got port={c.port}"
        )
    ...
    def test_missing_yaml_falls_back(self, tmp_path):
    ...
    def test_yaml_extras_are_ignored(self, tmp_path):
    ...
    def test_cli_none_is_treated_as_unset(self, tmp_path, monkeypatch):
        ...
        assert c.port == 8888, (
            "instance-config-resolve: CLI=None MUST yield to env; "
            f"got port={c.port}"
        )


class TestInstanceConfigEnvRoundTrip:
    def test_serialize_then_from_env(self, monkeypatch):
    ...
    def test_from_env_missing_is_fatal(self, monkeypatch):
