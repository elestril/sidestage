...
6. **Verify Feature Validation:** 
   - Ensure that all acceptance criteria and documented features for the task are fully covered by tests.
   - Use the `runLongCommand` to keep a server running at all times with the .workdir/dev 
   campaign. Use that to validate the behavior.
...
8. **Prepare for User Commit:**
   - Stage all code changes related to the task.
   - Propose a clear, concise commit message e.g, `feat(ui): Create basic HTML structure for calculator`.
   - **Inform the user that the code is ready for them to commit.**

9. **Attach Task Summary with Git Notes (After User Commit):**
   - **Step 9.1: Get Commit Hash:** After the user confirms they have committed the changes, obtain the hash of the *just-completed commit* (`git log -1 --format="%H"`).
...
11. **Finalize Task:** Inform the user that the task is complete in the plan and the summary has been attached as a git note.
...
2.  **Ensure Test Coverage for Phase Changes:**
    -   **Step 2.1: Determine Phase Scope:** Identify the files changed in this phase by checking the Git history since the last checkpoint.
    -   **Step 2.2: Verify and Create Tests:** For each modified code file, verify a corresponding test file exists and validates the functionality described in this phase's tasks (`plan.md`).

3.  **Execute Automated Tests with Proactive Debugging:**
    -   Run the automated test suite to verify the phase.
...
6.  **Attach Auditable Verification Report using Git Notes:**
    -   After the user performs the checkpoint commit, create a detailed verification report and attach it using `git notes` to that commit.

7.  **Get and Record Phase Checkpoint SHA:**
    -   **Step 7.1: Get Commit Hash:** Obtain the hash of the user's checkpoint commit.
...
8.  **Announce Completion:** Inform the user that the phase is complete and the summary has been recorded.
